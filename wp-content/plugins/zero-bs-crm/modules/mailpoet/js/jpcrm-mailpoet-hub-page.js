...
function jpcrm_mailpoet_initiate_sync(){
	...
	// initiate
	jpcrm_mailpoet_fire_sync( function( response ) {
		...
		*/
		...
		if ( typeof response.status != "undefined" ){
			...
			if ( response.total_crm_contacts_from_mailpoet ) {
				...
				document.getElementById('jpcrm-mailpoet-stat-contacts-synced').innerHTML = response.total_crm_contacts_from_mailpoet;
			...
			switch ( response.status ){
				...
				case 'sync_part_complete':
					...
					document.getElementById('jpcrm-mailpoet-status-short-text').textContent = response.status_short_text + ' (' + Math.round(response.percentage_completed) + '%)';
				...
				case 'sync_completed':
					...
					document.getElementById('jpcrm-mailpoet-status-icon').className = 'icon thumbs up green';
					document.getElementById('jpcrm-mailpoet-status-short-text').textContent = response.status_short_text;
					document.getElementById('jpcrm-mailpoet-status-long-text').textContent = response.status_long_text;
					return;
				...
				case 'not_in_ready_mode':
					...
					document.getElementById('jpcrm-mailpoet-status-icon').className = 'icon settings orange';
					document.getElementById('jpcrm-mailpoet-status-short-text').textContent = response.status_short_text;
					document.getElementById('jpcrm-mailpoet-status-short-text').className = 'status orange';
					document.getElementById('jpcrm-mailpoet-status-long-text').textContent = response.status_long_text;
					return;
			...
			if ( remaining_pages > 0 ){
					...
					title = jpcrm_mailpoet_language_label( 'pages_remain', '{0} pages remain' ).format( remaining_pages );
			...
			if ( typeof window.jpcrm_mailpoet_sync_blocked_runs == "undefined" || window.jpcrm_mailpoet_sync_blocked_runs < 10 ){
					...
					error_string = jpcrm_mailpoet_language_label( error_type, 'Import job is running in the back end. If this message is still shown after some time, please contact support.' );
			...
			if ( response.statusText == 'timeout' ) {
					...
					error_string = jpcrm_mailpoet_language_label( error_type, 'There was a general server error.' ) + ' (' + response.status + ')';
...
function jpcrm_mailpoet_language_label( key, fallback ){
	...
	if ( typeof window.jpcrm_mailpoet_language_labels != "undefined" && typeof window.jpcrm_mailpoet_language_labels[ key ] != "undefined" ) {
