...
function jpcrm_segment_export_to_mailpoet( exportButton ) {
    ...
    jpcrm_mailpoet_retrieve_list_summary( segment_name, function( response ){
        ...
        jQuery.ajax( {
            ...
            success: function ( response ) {
                ...
                if ( response.success == true && response.mailpoet_list_ID ) {
                    ...
                    jQuery('.swal2-cancel').off('click').on('click', () => {
...
function jpcrm_segment_batch_export( params ) {
    ...
    jQuery('.jpcrm-export-progress').html( `
        ...
        <div class="jpcrm-export-progress-bar" style="width:${ Math.floor((processing_in_current_batch * 100) / total) }%"></div>
    ` );
    ...
    jQuery.ajax( {
        ...
        success: function ( response ) {
            ...
            if ( response.is_last_batch ) {
                ...
                var completeTitle = jQuery('#jpcrm_segment_mailpoet_modal').attr('data-complete-title');
