				...
				<tbody>
					...
					<tr>
						<td class="wfieldname">
							...
							<?php esc_html_e( 'Here you can choose how you want to map WooCommerce order statuses to CRM contact statuses (if the above setting is enabled)', 'zero-bs-crm' ); ?>
						</td>
						<td style="width:540px">
							<table style="width:100%">
								<tr>
									<th><?php esc_html_e( 'Order status', 'zero-bs-crm' ); ?></th>
									<th><?php esc_html_e( 'Contact status', 'zero-bs-crm' ); ?></th>
								</tr>
								<?php
								...
								foreach ( $woo_order_statuses as $k => $v ) {
									...
									<tr class="jpcrm_woosync_order_status_map">
										...
										<td>
											<select class="winput" name="<?php echo esc_attr( $k ); ?>" id="<?php echo esc_attr( $k ); ?>">
												...
												foreach ( $contact_statuses as $status ) {
													echo '<option value="' . esc_attr( $status ) . '"' . ( $selected === $status ? ' selected' : '' ) . '>' . esc_html( $status ) . '</option>';
												}
												?>
											</select>
										</td>
