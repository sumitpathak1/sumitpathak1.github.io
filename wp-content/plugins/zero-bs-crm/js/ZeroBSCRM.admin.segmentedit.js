...
function zeroBSCRMJS_segment_buildConditionLine(rule){
	...
	// if rule == false it's an empty new one
		...
		if (typeof window.jpcrm_available_conditions != "undefined"){
			...
			if ( rule !== false && typeof window.jpcrm_available_conditions != "undefined" && typeof window.jpcrm_available_conditions[ rule.type ] == "undefined" && typeof window.jpcrm_available_conditions[ rule_type ] == "undefined" ){
				...
				jQuery.each( window.jpcrm_available_conditions_by_category, function( index, category ){
					...
					if ( typeof category.conditions != "undefined" && Object.keys(category.conditions).length > 0 ){
						...
						jQuery.each( ordered_conditions, function( condition_key, condition ){
							...
							if ( rule_type == condition.fieldname || ( rule.type && rule.type == condition.fieldname) || 'zbsc_' + rule_type == condition.fieldname ) html += ' selected="selected"';
		...
		if (typeof selected != "undefined"){
			...
			switch (selected){
				...
				case 'after':
					...
					html += '<input type="text" class="jpcrm-date-time zbs-segment-edit-var-condition-value" data-date-picker-format="YYYY-MM-DD HH:mm" value="' + jpcrm.esc_attr(v) + '" />';
				...
				case 'afterequal':
					...
					html += '<input type="text" class="jpcrm-date zbs-segment-edit-var-condition-value" data-date-picker-format="YYYY-MM-DD" value="'+jpcrm.esc_attr(v)+'" />';
				...
				case 'daterange':
					...
					html += '<input type="text" class="jpcrm-date-range zbs-segment-edit-var-condition-value" data-date-picker-format="YYYY-MM-DD" value="' + date_value + '" />';
				...
				case 'datetimerange':
					...
					html += '<input type="text" class="jpcrm-datetime-range zbs-segment-edit-var-condition-value" data-date-picker-format="YYYY-MM-DD HH:mm" value="' + date_value + '" />';
					...
					html += '<select class="zbs-segment-edit-var-condition-value">';
						...
						if ( window.jpcrm_mailpoet_status_list && window.jpcrm_mailpoet_status_list.length > 0 ){
							...
							for (var i=0; i<window.jpcrm_mailpoet_status_list.length; i++) {
								...
								html += '<option value="' + cur_status['key'] + '"' + (v === cur_status['key'] ? ' selected="selected"' : '') + '>' + cur_status['name'] + '</option>';

							}

						} else {

							html += '<option value="">' + zeroBSCRMJS_segmentLang('no_mailpoet_statuses') + '</option>';
						}
